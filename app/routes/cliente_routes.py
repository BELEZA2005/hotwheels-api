...
@router.get("/", response_model=List[ClienteRead], tags=["Clientes"])
def list_clientes(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1)):
    return get_clientes(skip=skip, limit=limit)


@router.get("/{cliente_id}", response_model=ClienteRead, tags=["Clientes"])
...
@router.post("/", response_model=ClienteRead, status_code=status.HTTP_201_CREATED, tags=["Clientes"])
...
@router.put("/{cliente_id}", response_model=ClienteRead, tags=["Clientes"])
...
@router.delete("/{cliente_id}", status_code=status.HTTP_204_NO_CONTENT, tags=["Clientes"])
